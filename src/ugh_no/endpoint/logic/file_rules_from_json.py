...
def load_rules_based_file() -> UghNoResponse | None:
    try:
        with open(MODEL_PATH_RULE_JSON, "r") as rule_json_file:
            ...
            logging.info("Loaded rules-based JSON file successfully.")
            if "rules_responses" in rules_json_read:
                ...
                logging.info("Found matching response:", filtered_data)

                if len(filtered_data) > 0:
                    random_chosen = random.choice(filtered_data)
                    response_obj = UghNoResponse(**random_chosen)
                    logging.info(f"Deserialized json to Python object {response_obj}")
                    return response_obj
        ...
        logging.error(f" Function load_rules_based_file -> File not found!: {str(e)}")
